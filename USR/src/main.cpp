...
namespace MOS
{
	...
	namespace Tasks
	{
		...
		int8_t Task0(void* argv)
		{
			while (true) {
				delay(500);
				leds[0].toggle();
				uart.send_string("[MOS]: Task0 running.\n");
			}
		}
		...
		int8_t Task1(void* argv)
		{
			while (true) {
				delay(750);
				leds[1].toggle();
				uart.send_string("[MOS]: Task1 running.\n");
			}
		}
		...
		int8_t Task2(void* argv)
		{
			while (true) {
				delay(1000);
				leds[2].toggle();
				uart.send_string("[MOS]: Task2 running.\n");
			}
		}
		...
		int8_t Task3(void* argv)
		{
			while (true) {
				delay(1500);
				uart.send_string("[MOS]: Task3 running.\n");
			}
		}
	}
	...
	namespace GloBalRes
	{
		...
		// Add more tasks here...
		TCB::Fn_t tasks[] = {Task0, Task1, Task2, Task3};
	}

	namespace Macro
	{
		using GloBalRes::tasks;
		constexpr uint32_t THREAD_NUM = sizeof(tasks) / sizeof(TCB::Fn_t);
		constexpr uint32_t STACK_SIZE = 64U;// STACK_SIZE >= 32-Bytes
	}
	...
	namespace GloBalRes
	{
		...
		TCB TCBList[THREAD_NUM];
		uint32_t TCB_STACK[THREAD_NUM][STACK_SIZE];
	}
	...
	struct Scheduler
	{
		static void thread_init()
		{
			...
			using GloBalRes::TCBList;
			using GloBalRes::TCB_STACK;
			using Macro::THREAD_NUM;
			...
			for (uint32_t i = 0; i < THREAD_NUM; ++i) {
				...
				TCBList[i].next = &TCBList[(i + 1) % THREAD_NUM];
				...
				TCBList[i].sp = &TCB_STACK[i][STACK_SIZE - 16];
				...
				TCB_STACK[i][STACK_SIZE - 2] = (uint32_t) tasks[i];
			}

			curTCB = &TCBList[0];// Make current tcb pointer point to task0
